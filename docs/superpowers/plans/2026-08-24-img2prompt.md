...
function validDoc(): Record<string, any> {
  return {
    ...
    text_in_image: [{ value: null, salience: 3, evidence: 'observed' }],
    exclusions: ['文字', '水印'],
  };
}
...
无文字时给一条 `value: null`。文字渲染是部分模型的强项，漏填等于放弃该能力。
...
| `text_in_image` | 单独一句，文字加引号：`The word "SALE" appears in bold condensed sans across the lower third.` null 时整句省略 |
...
| `text_in_image` | 引号写出文字内容 | 改字时写"replace the text with …" |
...
| `text_in_image` | 引号内文字 + 字体短语。null 时省略。**MJ 文字能力弱，此处应提醒用户** |
...
`text_in_image` 非 null 时，在输出后附一句：Midjourney 的文字渲染不可靠，该需求建议改用 GPT 或 Ideogram。**不要假装它能做到。**
````
...
| `text_in_image` | SD 文字能力弱。非 null 时照写并提醒用户 |
